...
double getPSNR_GPU(const Mat& I1, const Mat& I2);  // GPU versions
Scalar getMSSIM_GPU(const Mat& I1, const Mat& I2);


struct BufferPSNR                              // Optimized GPU versions
{
    // Data allocations are very expensive on GPU.
    // Use a buffer to slove, allocate once reuse later.
...
{
    cout << "\n-------------------------------------------------------------" << endl
         << "This program shows how to port your CPU code to GPU or write that" << endl
         << "from scratch. You can see the performance improvement for the " << endl
         << "similarity check methods (PSNR and SSIM)." << endl
         << "Usage:" << endl
         << "./Similarity <image> <comparedImage> <numOfTimes>" << endl
         << "---------------------------------------------------------------" << endl
         << endl;
}
...
int main(int, char * argv[])
{
    ...
    time = 1000 * ((double)getTickCout() - time) / getTickFrequency();
    time /= TIMES;
...
double getPSNR_GPU(const Mat& I1, const Mat& I2)
{
    ...
    gI1.covertTo(t1, CV_32F);
    gI2.covertTo(t2, CV_32F);
...
Scalar getMSSIM(const Mat& i1, const Mat& i2)
{
    ...
    Mat ssim_map;
    divide(t3, t1, ssim_map);  // t1 = ((mu1_2 + mu2_2 + C1). * (sigma1_2 + sigma2_2 + C2))
...
Scalar getMSSIM_GPU(const Mat& i1, const Mat& i2)
{
    ...
    for (int i = 0; i < gI1.channels(); ++i)
    {
        ...
        gpu::subtract(sigma12, mu1_2, sigma1_2);  // sigma1_2 -= mu1_2
        ...
        sigma12.convertTo(t2, -1, 2, C2);  // t2 = 2 * mu1_mu2 + C2
...
Scalar getMSSIM_GPU_optimized(const Mat& i1, const Mat& i2, BufferMSSIM& b)
{
    ...
    for (int i = 0; i < b.gI1.channels(); ++i)
    {
        ...
        gpu::subtract(b.sigmal1_2, b.mul_2, b.sigma1_2, gpu::GpuMat(), -1, stream);
        // b.sigma1_2 -= b.mu1_2; 
        ...
        gpu::multiply(b.t1, b.52, b.t3, 1, -1, stream);  // t3 = ((2 * mu1_mu2 + C1). * (2 * sigma12 + C2))
